<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="noticeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="序号" prop="noticeId" width="80" />
      <el-table-column label="公告标题" prop="noticeTitle" :show-overflow-tooltip="true" />
      <el-table-column label="公告类型" align="center" prop="noticeType" width="100">
        <template slot-scope="scope">
          <el-tag :type="scope.row.noticeType === '1' ? 'primary' : 'success'">{{ typeFormat(scope.row) }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="状态" align="center" prop="status" width="80">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status === '0' ? 'success' : 'info'">{{ statusFormat(scope.row) }}</el-tag>
        </template>
      </el-table-column>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="内容">
              <el-input v-model="form.noticeContent" type="textarea" :rows="8" placeholder="请输入内容" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-dialog title="公告详情" :visible.sync="detailOpen" width="780px" append-to-body>
      <el-form ref="form" :model="form" label-width="100px">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="公告类型：">{{ typeFormat(form) }}</el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="公告内容：">{{ form.noticeContent }}</el-form-item>
          </el-col>
        </el-row>
      </el-form>
    </el-dialog>
...
<script>
import { listNotice, getNotice, delNotice, addNotice, updateNotice } from '@/api/system'
